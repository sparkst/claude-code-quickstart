...
import type { ProcessInfo, ProcessState } from "./e2e-types.js";
...
describe("REQ-205 — Process Manager Resource Management", () => {
  ...
  describe("resource_leaks", () => {
    test("REQ-205 — prevents hanging processes after test completion", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        spawnPromises.push(
          processManager.spawn("sleep", [`${10 + i}`], { 
            timeout: 1000,
            tag: `test-sleep-${i}`
          })
        );
    ...
    test("REQ-205 — handles concurrent process creation without race conditions", async () => {
      ...
      for (let i = 0; i < processCount; i++) {
        concurrentOperations.push(
          processManager.spawn("echo", [`process-${i}`], {
            timeout: 2000,
            tag: `concurrent-${i}`
          })
        );
      ...
      processes.forEach(proc => {
        ...
        expect(proc.state).toBeOneOf(["running", "completed"] as ProcessState[]);
      });
      
      // All processes should complete successfully
      const results = await Promise.all(
        processes.map(proc => processManager.waitForCompletion(proc.pid))
      );
  ...
  describe("poor_error_handling", () => {
    test("REQ-205 — gracefully handles process spawn failures", async () => {
      ...
      await expect(
        processManager.spawn(nonExistentCommand, [], { timeout: 1000 })
      ).rejects.toThrow(/command not found|spawn.*failed/i);
  ...
  describe("race_conditions", () => {
    test("REQ-205 — prevents race conditions in concurrent cleanup operations", async () => {
      ...
      const activeProcesses = await processManager.getActiveProcesses();
      const ourProcesses = activeProcesses.filter(pid => 
    ...
    test("REQ-205 — thread-safe process tracking across concurrent operations", async () => {
      ...
      for (let i = 0; i < 20; i++) {
        if (i % 2 === 0) {
          ...
          operationPromises.push(
            processManager.kill(999999 + i).catch(() => null)
          );
        }
      }
      ...
      const activeProcesses = await processManager.getActiveProcesses();
      const ourActiveProcesses = activeProcesses.filter(pid => 
        processIds.includes(pid)
      );
