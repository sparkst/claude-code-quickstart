...
import { createCliExecutor, type CliExecutor } from "./cli-executor.js";
import type { ExecutionResult, ExecutionOptions } from "./e2e-types.js";
...
describe("REQ-202 — CLI Executor Real Process Execution", () => {
  ...
  describe("simulation_instead_of_real_execution", () => {
    test("REQ-202 — removes simulation and uses real subprocess spawning", async () => {
      ...
      const result: ExecutionResult = await cliExecutor.execute(testCommand, options);
  ...
  describe("poor_error_handling", () => {
    ...
    test("REQ-205 — properly cleans up child processes on error", async () => {
      ...
      const newSleepProcesses = pidsAfter.filter(pid => 
        !pidsBefore.includes(pid) && isProcessSleep(pid)
      );
  ...
  describe("security_vulnerabilities", () => {
    test("REQ-203 — prevents command injection attacks", async () => {
      ...
      for (const [command, ...args] of maliciousCommands) {
        ...
        await expect(
          cliExecutor.execute([command, ...args], {})
        ).rejects.toThrow(/unsafe|security|invalid/i);
      }
    });
    ...
    test("REQ-203 — validates executable paths to prevent privilege escalation", async () => {
      ...
      for (const executable of dangerousExecutables) {
        await expect(
          cliExecutor.execute([executable, "--help"], {})
        ).rejects.toThrow(/unsafe.*path|invalid.*executable/i);
      }
    });
    ...
    test("REQ-203 — sanitizes environment variables", async () => {
      ...
      await expect(
        cliExecutor.execute(["node", "--version"], { env: maliciousEnv })
      ).rejects.toThrow(/unsafe.*environment|invalid.*env/i);
    });
  });
});

// Helper functions that should exist in the implementation
async function getActiveProcessIds(): Promise<number[]> {
  try {
...
function isProcessSleep(pid: number): boolean {
  try {
