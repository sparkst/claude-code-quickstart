...
import os from 'os';
import { createE2EEnvironment, destroyE2EEnvironment, registerCleanupCallback } from './process-manager.js';
...
export class E2EEnvironmentManager {
    ...
    async createEnvironment(options = {}) {
        const { prefix = 'claude-test-e2e-', mcpServers = ENVIRONMENT_CONFIG.DEFAULT_MCP_SERVERS, createClaudeConfig = true } = options;
        try {
            ...
            if (createClaudeConfig) {
                ...
                await fs.writeFile(baseEnvironment.claudeConfigPath, JSON.stringify(config, null, 2));
            }
            ...
            throw new Error(`Failed to create E2E environment: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    async updateClaudeConfig(environmentId, config) {
        const environment = this.getEnvironment(environmentId);
        if (!environment) {
            throw new Error(`Environment ${environmentId} not found`);
        }
        try {
            await fs.writeFile(environment.claudeConfigPath, JSON.stringify(config, null, 2));
        }
        catch (error) {
            throw new Error(`Failed to update Claude config: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    async readClaudeConfig(environmentId) {
        const environment = this.getEnvironment(environmentId);
        if (!environment) {
            throw new Error(`Environment ${environmentId} not found`);
        }
 ...
 */
export async function createStandardTestEnvironment() {
 ...
 */
export async function createCustomTestEnvironment(mcpServers) {
    return environmentManager.createEnvironment({
 ...
 */
export async function setupTestSuite(suiteName) {
